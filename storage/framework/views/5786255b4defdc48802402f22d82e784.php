<?php $__env->startSection('title', 'Manajemen Stok Menu & Bahan Baku — Koki SIResto'); ?>
...
<div class="space-y-6">
    ...
    <div class="bg-slate-900 text-white p-5 rounded-2xl shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex items-center space-x-2">
            <a href="<?php echo e(route('koki.kds')); ?>" class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold rounded-xl border border-slate-700 transition">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <?php $__currentLoopData = $allMenu; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $menu): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="p-4 rounded-xl border transition flex flex-col justify-between space-y-3
                    <?php echo e($menu->status_ketersediaan === 'tersedia' ? 'bg-white border-slate-200 shadow-sm' : 'bg-red-50 border-red-200 shadow-sm'); ?>">
                    <div>
                        ...
                        <h3 class="font-bold text-slate-900 text-base leading-snug"><?php echo e($menu->nama_menu); ?></h3>
                        <p class="text-xs font-mono font-bold text-emerald-600 mt-1">Rp <?php echo e(number_format($menu->harga, 0, ',', '.')); ?></p>
                    </div>

                    <div class="pt-2 border-t border-slate-100 flex items-center justify-between">
                        <span class="text-xs font-extrabold px-2.5 py-1 rounded-full
                            <?php echo e($menu->status_ketersediaan === 'tersedia' ? 'bg-emerald-100 text-emerald-800' : 'bg-red-600 text-white'); ?>">
                            <?php echo e($menu->status_ketersediaan === 'tersedia' ? '🟢 Tersedia' : '🔴 Habis (Bahan Kosong)'); ?>

                        </span>

                        <form action="<?php echo e(route('koki.menu.toggle', $menu->kode_menu)); ?>" method="POST">
                            <?php echo csrf_field(); ?>
                            <?php echo method_field('PATCH'); ?>
                            <button type="submit" class="text-xs font-bold px-3 py-1.5 rounded-lg border transition
                                <?php echo e($menu->status_ketersediaan === 'tersedia' ? 'bg-red-100 text-red-700 hover:bg-red-200 border-red-300' : 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200 border-emerald-300'); ?>">
                                <?php echo e($menu->status_ketersediaan === 'tersedia' ? 'Set Habis ❌' : 'Set Tersedia ✅'); ?>
        ...
        <div class="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-4 h-fit sticky top-20 shadow-sm">
            ...
            <form action="<?php echo e(route('koki.menu.store')); ?>" method="POST" class="space-y-3">
                <?php echo csrf_field(); ?>
                <div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ArtvAdmirer\.gemini\antigravity\scratch\siresto\resources\views/koki/menu_management.blade.php ENDPATH**/ ?>