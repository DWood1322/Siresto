...
@section('title', 'Kasir POS & Pelunasan — SIResto')
...
<div class="space-y-6">
    ...
    <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <span class="bg-blue-600 text-white font-bold text-xs px-3 py-1.5 rounded-xl shadow-sm">
            {{ $pesananSiapBayar->count() }} Tagihan Menunggu
        </span>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @forelse($pesananSiapBayar as $order)
                    @php
                    ...
                    <div class="order-card bg-white p-5 rounded-2xl shadow-sm border border-slate-200 hover:border-blue-500 transition cursor-pointer flex flex-col justify-between"
                        onclick="selectOrder('{{ $order->no_pesanan }}', '{{ $order->no_meja }}', '{{ $totalTagihan }}')">
                        <div>
                            <div class="flex justify-between items-start mb-2">
                                <span class="text-lg font-black text-slate-900">Meja #{{ $order->no_meja }}</span>
                                <span class="text-[10px] font-bold uppercase px-2 py-0.5 rounded
                                    {{ $order->status_pesanan === 'siap_disajikan' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800' }}">
                                    {{ $order->status_pesanan }}
                                </span>
                            </div>
                            <p class="text-xs text-slate-400 font-mono mb-2">{{ $order->no_pesanan }}</p>
                            <p class="text-xs text-slate-600 mb-3">Tamu: {{ $order->pelanggan->jumlah_tamu ?? 1 }} org | Items: {{ $order->details->count() }} jenis</p>
                        </div>

                        <div class="pt-3 border-t border-slate-100 flex justify-between items-center">
                            <span class="text-xs text-slate-500 font-medium">Total Tagihan</span>
                            <span class="font-black font-mono text-blue-600 text-lg">Rp {{ number_format($totalTagihan, 0, ',', '.') }}</span>
                        </div>
                    </div>
                @empty
                ...
                <div class="space-y-3 max-h-60 overflow-y-auto pr-1">
                    @forelse($transaksiHariIni as $trx)
                        <div class="flex items-center justify-between p-3 bg-slate-50 border border-slate-100 rounded-xl text-xs">
                            <div>
                                <span class="font-bold text-slate-900 font-mono">{{ $trx->no_transaksi }}</span>
                                <p class="text-slate-500 text-[11px]">No Pesanan: {{ $trx->no_pesanan }} | Metode: {{ strtoupper($trx->metode_bayar) }}</p>
                            </div>
                            <div class="text-right">
                                <span class="font-bold font-mono text-emerald-600 text-sm">Rp {{ number_format($trx->total_tagihan, 0, ',', '.') }}</span>
                                <p class="text-[10px] text-slate-400">Kembalian: Rp {{ number_format($trx->kembalian, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    @empty
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col justify-between space-y-6 h-fit sticky top-20">
            <div>
                ...
                <form id="checkoutForm" action="{{ route('kasir.payment.process') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
