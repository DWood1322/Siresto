...
@section('title', 'Order POS Pelayan — SIResto')
...
<div class="space-y-6">
    ...
    <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex items-center space-x-3 text-xs">
            <span class="bg-emerald-100 text-emerald-800 font-bold px-3 py-1.5 rounded-lg border border-emerald-300 flex items-center space-x-1">
                <span>🔔</span>
                <span>Siap Diantar: <strong>{{ $pesananSiapDisajikan->count() }} Meja</strong></span>
            </span>
            <span class="bg-slate-100 text-slate-700 px-3 py-1.5 rounded-lg border font-medium">
                Meja Kosong: <strong>{{ $meja->count() }}</strong> / {{ $allMeja->count() }}
            </span>
        </div>
    </div>

    <!-- NOTIFIKASI INFORMASI MASAKAN SIAP DISAJIKAN -->
    @if($pesananSiapDisajikan->count() > 0)
        <div class="bg-emerald-900 text-white p-6 rounded-2xl shadow-md space-y-4">
            <div class="flex justify-between items-center border-b border-emerald-800 pb-3">
                <div class="flex items-center space-x-2">
                    ...
                    <h2 class="text-lg font-black text-emerald-300">INFO DAPUR: {{ $pesananSiapDisajikan->count() }} Pesanan Selesai Dimasak!</h2>
                </div>
                <a href="{{ route('pelayan.ready') }}" class="text-xs bg-emerald-800 hover:bg-emerald-700 text-emerald-100 px-3 py-1 rounded-full font-bold">
    ...
    <form action="{{ route('pelayan.order.store') }}" method="POST" id="posForm" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        @csrf
            ...
            <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="no_meja" required class="w-full border border-slate-300 rounded-xl p-3 bg-slate-50 text-sm font-semibold focus:ring-2 focus:ring-amber-500 focus:bg-white transition">
                        ...
                        @foreach($meja as $m)
                            <option value="{{ $m->no_meja }}">Meja {{ $m->no_meja }} (Kapasitas: {{ $m->kapasitas_kursi }} Kursi)</option>
                        @endforeach
                    </select>
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach($menuList as $index => $menu)
                    ...
                    <div class="menu-card rounded-2xl shadow-sm p-4 border flex flex-col justify-between transition
                        {{ $isHabis ? 'bg-slate-100 border-red-200 text-slate-400 opacity-80' : 'bg-white border-slate-200 hover:border-amber-400 text-slate-900' }}"
                        ...
                        <div>
                            ...
                            <h3 class="font-bold text-base leading-snug mb-1 {{ $isHabis ? 'line-through text-slate-500' : 'text-slate-900' }}">
                                {{ $menu->nama_menu }}
                            </h3>
                            <p class="font-extrabold font-mono text-base mb-3 {{ $isHabis ? 'text-slate-400' : 'text-amber-600' }}">
                        ...
                        <div class="space-y-2 pt-2 border-t border-slate-200">
                            <input type="hidden" name="items[{{ $index }}][kode_menu]" value="{{ $menu->kode_menu }}">
                            <div class="flex items-center justify-between p-1.5 rounded-xl border {{ $isHabis ? 'bg-slate-200 border-slate-300' : 'bg-slate-50 border-slate-200' }}">
                                <button type="button" onclick="updateQty({{ $index }}, -1)" {{ $isHabis ? 'disabled' : '' }}
                                    class="w-8 h-8 rounded-lg bg-white border border-slate-300 font-bold text-slate-700 flex items-center justify-center transition {{ $isHabis ? 'opacity-40 cursor-not-allowed' : 'hover:bg-slate-200' }}">-</button>

                                <input type="number" id="qty_{{ $index }}" name="items[{{ $index }}][jumlah]" value="0" min="0" readonly
                                    class="w-12 text-center bg-transparent font-extrabold text-sm {{ $isHabis ? 'text-slate-400' : 'text-slate-900' }} focus:outline-none"
                                    data-price="{{ $menu->harga }}" data-name="{{ $menu->nama_menu }}" data-status="{{ $menu->status_ketersediaan }}">

                                <button type="button" onclick="updateQty({{ $index }}, 1)" {{ $isHabis ? 'disabled' : '' }}
                                    class="w-8 h-8 rounded-lg font-bold text-white flex items-center justify-center shadow-sm transition {{ $isHabis ? 'bg-slate-400 opacity-40 cursor-not-allowed' : 'bg-amber-500 hover:bg-amber-600' }}">+</button>
                            </div>

                            @if($isHabis)
                                <p class="text-[10px] font-bold text-red-600 italic text-center py-1">⚠️ Bahan baku di dapur kosong</p>
                            @else
                                <input type="text" name="items[{{ $index }}][catatan]" placeholder="Catatan kustom (misal: pedas)"
